...
function(VisCore_add_module module_name)
  ...
  cmake_parse_arguments(MD "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${ARGN})
  ...
  if(NOT MD_INTERFACE)
    ...
    # 递归获取所有 .c 和 .cpp 文件
    file(GLOB_RECURSE module_sources_src "${module_source_dir}/*.c"
  ...
  if(MD_INTERFACE)
    ...
    target_include_directories(
      ${module_lib_name} INTERFACE ${CMAKE_CURRENT_LIST_DIR}/include
                                   ${MD_EXTRA_HEADER})
    # VisCore公共模块依赖头文件
    foreach(depends_modules ${MD_DEPENDS})
      ...
      target_link_libraries(${module_lib_name}
                            INTERFACE ${depends_module_lib_name})
    endforeach()
    # 添加第三方库依赖
    ...
    # 添加头文件和第三方库依赖头文件
    target_include_directories(
      ${module_lib_name} PUBLIC ${CMAKE_CURRENT_LIST_DIR}/include
                                ${MD_EXTRA_HEADER})
    # VisCore公共模块依赖头文件
    foreach(depends_modules ${MD_DEPENDS})
      set(depends_module_lib_name "${module_prefix}${depends_modules}")
      target_link_libraries(${module_lib_name}
                            PUBLIC ${depends_module_lib_name})
    endforeach()
    # 添加第三方库依赖
    foreach(external_lib ${MD_EXTERNAL})
  ...
  # 记录构建的模块
  if(NOT MD_INTERFACE)
    set(VisCore_MODULES_PUBLIC
        ${VisCore_MODULES_PUBLIC} ${module_lib_name}
        CACHE INTERNAL "VisCore公共模块" FORCE)
  else()
    set(VisCore_MODULES_INTERFACE
        ${VisCore_MODULES_INTERFACE} ${module_lib_name}
        CACHE INTERNAL "VisCore接口模块" FORCE)
  endif()
  set(VisCore_MODULES_BUILD
      ${VisCore_MODULES_BUILD} ${module_lib_name}
      CACHE INTERNAL "VisCore构建库" FORCE)
endfunction()
...
function(VisCore_add_exe exe_name)
  ...
  # 设置可执行文件的头文件搜索路径
  target_include_directories(${target_name} PUBLIC ${exe_include_dir}
                                                   ${EXE_EXTRA_HEADER})
