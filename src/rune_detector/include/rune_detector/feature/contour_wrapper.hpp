...
class ContourWrapper
{
 ...
 public:
   ...
   * @param contour 轮廓点集
   */
  ContourWrapper(const PointVec& contour) : points_(contour) {}
   ...
   * @param contour 轮廓点集
   */
  ContourWrapper(PointVec&& contour) noexcept(false) : points_(std::move(contour)) {}
 ...
 public:
   ...
   * @brief 获取轮廓点集的零拷贝只读
   */
  [[nodiscard]] std::span<const PointType> View() const noexcept { return points_; }

  /**
   * @brief 隐式转换为点集引用
   */
  [[nodiscard]] operator const PointVec&() const noexcept { return points_; }
   ...
   * @brief 获取轮廓周长
   */
  [[nodiscard]] double Perimeter(bool closed = true) const
  {
    if (closed)
    {
   ...
   * @brief 获取轮廓中心点
   */
  [[nodiscard]] KeyPointType Center() const
  {
    return center_.get(
        [this]
        {
          const cv::Scalar MEAN = cv::mean(points_);
          return KeyPointType(static_cast<KeyType>(MEAN[0]),
                              static_cast<KeyType>(MEAN[1]));
        });
  }
 ...
 public:
  using const_iterator = typename PointVec::const_iterator;  // 常量迭代器
  using const_reverse_iterator =
      typename PointVec::const_reverse_iterator;  // 常量反向迭代器
 ...
 *       类型不同时生成新轮廓（触发写时复制）
 */
template <ContourBaseType OutputType, ContourWrapperPtrType ContourPtr>
[[nodiscard]] inline auto convert(const ContourPtr& contour)
