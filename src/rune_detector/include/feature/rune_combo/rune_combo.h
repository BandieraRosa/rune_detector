...
class RuneCombo : public FeatureNode
{
 ...
 public:
   ...
   */
  RuneCombo(const FeatureNodePtr& p_target, const FeatureNodePtr& p_center,
            const FeatureNodePtr& p_fan, const PoseNode& rune_to_cam,
            const RuneType& type, const GyroData& gyro_data, int64 tick, const cv::Mat& k,
            const cv::Mat& d, const cv::Matx33d& cam2joint_rmat,
   ...
   */
  static Ptr MakeFeature(const PoseNode& pnp_data, const RuneType& type,
                         const GyroData& gyro_data, int64 tick, const cv::Mat& k,
                         const cv::Mat& d, const cv::Matx33d& cam2joint_rmat,
                         const cv::Vec3d& cam2joint_tvec);
   ...
   * @return 派生对象指针
   */
  static inline Ptr Cast(const FeatureNodePtr& p_combo)
  {
    return std::dynamic_pointer_cast<RuneCombo>(p_combo);
  }
  static inline const std::shared_ptr<const RuneCombo> Cast(const FeatureNodeConstPtr& p_combo)
  {
