...
class RuneCenter : public FeatureNode
{
 ...
 public:
   ...
   */
  RuneCenter(const ContourConstPtr& contour, cv::RotatedRect& rotated_rect);
   ...
   */
  static void Find(std::vector<FeatureNodePtr>& centers,
   ...
   */
  static std::shared_ptr<RuneCenter> MakeFeature(const cv::Point2f& center,
                                                 bool force = false);
   ...
   * @return 如果成功，返回 RuneCenter 的共享指针，否则返回 nullptr
   */
  static std::shared_ptr<RuneCenter> MakeFeature(
   ...
   */
  static std::shared_ptr<RuneCenter> MakeFeature(const PoseNode& center_to_cam,
   ...
   * @return 派生对象指针
   */
  static inline std::shared_ptr<RuneCenter> Cast(const FeatureNodePtr& p_feature)
  {
    return std::dynamic_pointer_cast<RuneCenter>(p_feature);
  }
  static inline const std::shared_ptr<const RuneCenter> Cast(
...
//! 神符中心特征共享指针
using RuneCenterPtr = std::shared_ptr<RuneCenter>;
using RuneCenterConstPtr = std::shared_ptr<const RuneCenter>;
