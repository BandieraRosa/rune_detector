...
class RuneFanHump
{
 ...
 public:
  ...
  {
    if (__ContourPtr == nullptr)
    {
      std::vector<cv::Point> contour;
      contour.push_back(center);
      contour.push_back(vertex);
      contour.push_back(center + direction * 20);
      __ContourPtr = ContourWrapper<int>::MakeContour(contour);
    }
    return __ContourPtr;
  }

 private:
  mutable ContourPtr __ContourPtr = nullptr;  //!< 调试用轮廓缓存
};
...
struct TopHumpCombo
{
  ...
  ContourConstPtr getContour() const
  {
    if (__ContourPtr == nullptr)
    {
      __ContourPtr = ContourWrapper<int>::GetConvexHull(
 ...
 private:
  ...
  mutable ContourPtr __ContourPtr;  //!< 调试用轮廓缓存
};
