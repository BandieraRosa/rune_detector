...
class RuneTargetInactive : public RuneTarget
{
 ...
 public:
   ...
   */
  RuneTargetInactive(const ContourConstPtr& contour,
   ...
   */
  static void Find(std::vector<FeatureNodePtr>& targets,
   ...
   */
  static Ptr MakeFeature(const PoseNode& target_to_cam, const cv::Mat& k,
                         const cv::Mat& d);
 ...
 protected:
   ...
   * @return 构造的 RuneTargetInactive 智能指针
   */
  static Ptr MakeFeature(PoseNode& target_to_cam, bool is_active);
 ...
 protected:
  ...
  DEFINE_PROPERTY_WITH_INIT(GapCorners, public, protected, (std::vector<cv::Point2f>),
                            std::vector<cv::Point2f>{});  //!< 缺口角点占位

  inline static const cv::Point2f VACANCY_POINT = cv::Point2f(-1, -1);  //!< 默认占位点
};

using RuneTargetInactivePtr = std::shared_ptr<RuneTargetInactive>;  //!< 可修改的智能指针
using RuneTargetInactiveConstPtr =
